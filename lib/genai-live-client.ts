...
export interface LiveClientEventTypes {
  ...
  grounding: (data: GroundingMetadata) => void;
...
export class GenAILiveClient {
  ...
  on<T extends keyof LiveClientEventTypes>(
    event: T,
  ...
  off<T extends keyof LiveClientEventTypes>(
    event: T,
    listener: LiveClientEventTypes[T],
  ): this {
  ...
  emit<T extends keyof LiveClientEventTypes>(
    event: T,
    ...args: Parameters<LiveClientEventTypes[T]>
  ): boolean {
  ...
  protected onMessage(message: LiveServerMessage) {
    ...
    if (message.serverContent) {
      ...
      if (serverContent.modelTurn) {
        const modelTurn = serverContent.modelTurn as Content & {
          groundingMetadata?: GroundingMetadata;
        };
        if (modelTurn.groundingMetadata) {
          this.emit('grounding', modelTurn.groundingMetadata);
          this.log(
            'server.grounding',
            modelTurn.groundingMetadata.groundingChunks,
          );
        }
        ...
        const content: LiveServerContent = {
          modelTurn: { parts, role: modelTurn.role },
        };
